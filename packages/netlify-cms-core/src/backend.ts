import { attempt, flatten, isError, uniq, groupBy, difference } from 'lodash';
    ...
    return this.implementation.unpublishedEntries!()
      .then(entries =>
        entries.map(loadedEntry => {
          ...
          const entry = createEntry(collectionName, loadedEntry.slug, loadedEntry.file.path, {
            ...
            contentKey: loadedEntry.contentKey,
      ...
      .then(entries => {
        const formattedEntries = entries.reduce((acc, entry) => {
          ...
          if (collection) {
            const formattedEntry = this.entryWithFormat(collection)(entry) as EntryValue;
            formattedEntry.i18nStructure &&
              (formattedEntry.slugWithLocale = selectEntrySlug(collection, formattedEntry.path));
            acc.push(formattedEntry);
          } else {
            console.warn(
        ...
        const multiContentEntries = formattedEntries.filter(e =>
          DIFF_FILE_TYPES.includes(e.i18nStructure),
        );
        const combinedMultiEntries = this.combineMultipleContentEntries(multiContentEntries);
        return {
          pagination: 0,
          entries: [...difference(formattedEntries, multiContentEntries), ...combinedMultiEntries],
        };
      });
  }
    ...
    return this.implementation!.unpublishedEntry!(collection.get('name') as string, slug)
      ...
      .then(entries => {
        if (DIFF_FILE_TYPES.includes(collection.get('i18n_structure'))) {
          return this.combineEntries(
            entries.map(e => ({ ...e, slugWithLocale: selectEntrySlug(collection, e.path) })),
          );
        }
